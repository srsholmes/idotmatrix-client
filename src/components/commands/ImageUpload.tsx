...
export function ImageUpload() {
  ...
  const handlePickFile = async () => {
    ...
    if (result) {
      ...
      setMessage(null);
      // Auto-preview
      const prev = await execute(["image", "preview", result as string, "--size", String(size)]);
  ...
  const handleUpload = async () => {
    ...
    const result = await execute(
      [...getBaseArgs(), "image", "upload", filePath, "--size", String(size)],
    ...
    <div className="max-w-lg space-y-4">
      ...
      >
        <p className="text-gray-400">
          {filePath ? filePath.split("/").pop() : "Click to select an image"}
        </p>
      </button>
